...
MUI.Desktop = {
	...
	setDesktopSize: function(){
		...
		//if ( MUI.Columns.instances.getKeys().length > 0){ // Conditional is a fix for a bug in IE6 in the no toolbars demo.
                if ( Object.keys(MUI.Columns.instances).length > 0){ // Conditional is a fix for a bug in IE6 in the no toolbars demo.
...
MUI.Column = new Class({
	...
	initialize: function(options){
		...
		var instanceID = instances.get(options.id); 
                //if(instances.options){
                //    var instanceID = instances.options.id;
                //}
	...
	columnToggle: function(){
		...
		// Expand
			...
			this.resize.attach();
			this.handleEl.setStyle('cursor', Browser.firefox ? 'col-resize' : 'e-resize').addClass('attached');
