...
Fx.Morpher = new Class({
	...
	start: function(properties, paused){
		...
		if(this.options.path && usepath.length) {
			...
			if(usepath.contains('top')) 
                            //$extend(properties, {top:[0,1]});
                            Object.append( {top:[0,1]}, properties);    
			if(usepath.contains('left')) 
                           // $extend(properties, {left:[0,1]});
                           Object.append( {left:[0,1]} , properties );    
		}
