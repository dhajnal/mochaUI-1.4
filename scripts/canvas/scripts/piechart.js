...
Dependencies:
        ...
        Modified for Mootools 1.4.5 by darko.hajnal@domenacom.hr , 2013-02-10
...
var PieChart = new Class({
	...
	insertElements: function(el){
		...
		//}).injectInside(this.container.getElement('.pieChartWrapper')).addClass('pieChartCanvas');
                }).inject(this.container.getElement('.pieChartWrapper')).addClass('pieChartCanvas');
		...
		}).inject(this.container ,'bottom').addClass('pieChartMap');

		// Insert the blank transparent gif that is used for the image map

		// Fix: must be relative folder
                new Asset.image( 'images/spacer.gif', {
                        alt: '',
			usemap: '#pieChartMap' + this.tableIndex,
			width: this.pieChartDiameter,
			height: this.pieChartDiameter
		}).inject(this.container.getElement('.pieChartWrapper')).setStyles({
				'width': this.pieChartDiameter,
				'height': this.pieChartDiameter
			});

		// Insert a div to insure layout integrity
		new Element('div').inject(this.container,'bottom').addClass('clear');
	...
	for (var i = 0; i < trs.length; i++) {
    	...
    	for (piece=0; piece < tableLength; piece++) {
			...
			}).inject(this.container.getElement('.pieChartMap'));
